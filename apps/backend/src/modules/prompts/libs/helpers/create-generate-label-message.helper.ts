import { LabelConstraint } from "~/modules/labels/labels.js";
...
const createGenerateLabelMessage = ({
	...
	return `Assign a single category label to the provided prompt. The label must be one-word
		...
		5. Return exactly one word: lower-case ASCII letters and digits only. No spaces, hyphens, underscores, punctuation, or quotes, and at most ${LabelConstraint.MAX_LENGTH.toString()} characters. Prefer a shorter common word over a longer precise one.`;
};
