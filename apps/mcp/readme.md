...
- `resolve_repository`: checks which workspace the current checkout is bound to, by reading its git remote. Takes no arguments. Call it before composing or searching prompts, to know the current workspace.
- `bind_repository`: binds the current checkout to a workspace, so future calls resolve to it. Call it after `resolve_repository` reports the checkout as unresolved or ambiguous, passing the `workspaceId` to bind to. Detects the project's technologies from `package.json` and records them on the workspace.
